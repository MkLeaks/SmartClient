...
public class NetworkManager extends SimpleChannelInboundHandler<Packet>
{
    ...
    private void dispatchPacket(final Packet inPacket, final GenericFutureListener <? extends Future <? super Void >> [] futureListeners)
    {
        ...
        if (inPacket instanceof C00Handshake) {
            if (Main.BungeeHack && !Main.SessionPremium && !Main.PremiumUUID && (
                    (C00Handshake)inPacket).getRequestedState() == EnumConnectionState.LOGIN) {
                ((C00Handshake)inPacket)
                        .setIp(String.valueOf(String.valueOf(((C00Handshake)inPacket).getIp())) + "\000" + Main.IpBungeeHack + "\000" +
                                UUID.nameUUIDFromBytes(("OfflinePlayer:" + Main.getFakeNick()).getBytes()).toString()
                                        .replace("-", ""));
                System.out.println(((C00Handshake)inPacket).getIp());
                System.out.println(Main.getFakeNick());
            }
            if (Main.BungeeHack && !Main.SessionPremium && Main.PremiumUUID && (
                    (C00Handshake)inPacket).getRequestedState() == EnumConnectionState.LOGIN) {
                ((C00Handshake)inPacket).setIp(String.valueOf(String.valueOf(((C00Handshake)inPacket).getIp())) + "\000" +
                        Main.IpBungeeHack + "\000" + Main.PreUUID);
                System.out.println(((C00Handshake)inPacket).getIp());
                System.out.println(mc.getSession().getUsername());
            }
            if (Main.BungeeHack && Main.SessionPremium && (
                    (C00Handshake)inPacket).getRequestedState() == EnumConnectionState.LOGIN) {
                ((C00Handshake)inPacket).setIp(
                        String.valueOf(String.valueOf(((C00Handshake)inPacket).getIp())) + "\000" + Main.IpBungeeHack + Main.PreUUID);
                System.out.println(((C00Handshake)inPacket).getIp());
                System.out.println(mc.getSession().getUsername());
            }
        }
